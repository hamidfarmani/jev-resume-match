...
export async function POST(request: Request) {
  if (!process.env.TYPESAFE_API_KEY && !process.env.JEV_API_KEY) {
    return Response.json({ error: "Set TYPESAFE_API_KEY on the server to enable reviews." }, { status: 503 });
  }
  try {
    ...
    const projects = field(data, "projects", 4_000);
    const languages = field(data, "languages", 1_000);
    ...
    const result = await reviewResume({ resume, targetRole, jobDescription, projects, languages, pageCount: extracted.pageCount });
